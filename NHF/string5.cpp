...
size_t String::hossz() const {
    
    size_t hossz = 0;
    while (this->pData[hossz] != '\0') {
        hossz++;
    }
    return hossz;
}
...
bool String::tartalmaz(const String& reszlet) const {
    if (reszlet.hossz() > this->hossz()) {
        return false;
    }
    ...
    for (size_t i = 0; i <= this->hossz() - reszlet.hossz(); ++i) {
        bool egyezik = true;
        for (size_t j = 0; j < reszlet.hossz(); ++j) {
            if (this->operator[](i + j) != reszlet[j]) {
                egyezik = false;
                break;
            }
        }
...
void String::beolvas(std::istream& is) {
    *this = String(""); // Üres stringre állítás

    char ch;
    while (is.get(ch) && ch != '\n') {
        *this = *this + ch; // String + char operátor
    }
}
